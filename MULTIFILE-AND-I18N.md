...
### 2.8 后续修复：版本名带上真实文件名（2026-09-15）

首版实现里，版本选择器的版本名只给「分辨率 + 编码」（如 `1080p H264`），文件名完全丢了——
合并场景的两个文件只要分辨率和编码相同，客户端就渲染成**两行一模一样的 `1080p H264`**，
用户无法分辨哪行播的是哪个文件，与本文档 §2.1 承诺的「文件名/分辨率作为版本名」不符。

修复后的命名规则（`mapping/scene.py::multi_file_source_title`，`format_jellyfin_item` 与
`endpoints/playback.py` 两处共用）：
...
- **主文件的 MediaSource 名也换成文件名**（仅多文件场景且开关开启时；单文件场景不受影响，
  characterization 基线不变）——否则用户无法知道默认那一行对应哪个文件。
- 客户端按 `Name` 区分版本行，因此**跨源去重是硬要求**，冲突会静默合并成一行。
- `version_display_name()` 保留，作为规格串（`1080p H264`）的底层构建器。
